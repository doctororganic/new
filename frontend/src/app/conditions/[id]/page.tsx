...
import { ConditionItemSchema } from '@/lib/schemas'

export default async function ConditionDetailPage({ params }: { params: { id: string } }) {
  const res = await fetch(`${apiBase}/v1/conditions/${params.id}`, { next: { revalidate: 0 } })
  if (!res.ok) return <main className="p-6"><pre className="bg-red-100 p-4 rounded">Status {res.status}</pre></main>
  const data = await res.json()
  const parsed = ConditionItemSchema.safeParse(data)
  if (!parsed.success) return <main className="p-6"><pre className="bg-yellow-100 p-4 rounded">Invalid schema</pre></main>
  return (
    <main className="p-6">
      <h2 className="text-2xl font-semibold mb-4">Condition Detail</h2>
