...
export default function WorkoutEditPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`${apiUrl}/v1/workouts/${params.id}`).then(r => r.json()).then(setInitial).catch(() => setInitial({ error: 'Load failed' }))
  }, [params.id])
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch(`${apiUrl}/v1/workouts/${params.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
  ...
  return (
    <main className="p-6">
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-2 mb-4">
        ...
        <input name="duration" defaultValue={initial?.duration ?? ''} placeholder="Duration" type="number" className="border p-2 rounded" />
        <input name="calories_burned" defaultValue={initial?.calories_burned ?? ''} placeholder="Calories burned" type="number" className="border p-2 rounded" />
        <input name="type" defaultValue={initial?.type || ''} placeholder="Type" className="border p-2 rounded" />
